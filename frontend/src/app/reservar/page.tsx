...
  const [isLoadingDisponibilidade, setIsLoadingDisponibilidade] = useState(false);
  ...
  const watchedDataReserva = useWatch({ control, name: "datadaReserva" });
...
    if (!watchedLivroId || !watchedDataReserva) {
...
          `${process.env.NEXT_PUBLIC_URL_API}/livros/${watchedLivroId}/disponibilidade?data=${watchedDataReserva}`
        );
...
  }, [watchedLivroId, watchedDataReserva]);
...
            </div>
            ...
            <div className="mb-5 h-6">
              {isLoadingDisponibilidade && (
                <p className="text-sm text-gray-600 dark:text-gray-400">Verificando disponibilidade...</p>
              )}
              ...
              {!isLoadingDisponibilidade && isDisponivel && watchedDataReserva && (
...
              {isLoadingDisponibilidade ? "Verificando..." : "Confirmar Reserva"}
            </button>
          </form>
        </div>
      </div>
